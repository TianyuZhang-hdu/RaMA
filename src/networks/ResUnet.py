...
class ResUnet(nn.Module):
    def __init__(self, resnet='resnet34', num_classes=2, pretrained=False, in_ch=3):
        super().__init__()
        if resnet == 'resnet34':
            base_model = resnet34
        ...
        if in_ch != 3:
            w = self.res.conv1.weight.data
            self.res.conv1 = nn.Conv2d(in_ch, feature_channels[0], kernel_size=7, stride=2, padding=3, bias=False)
            if w.shape[1] == 3 and in_ch == 1:
                self.res.conv1.weight.data = w.mean(dim=1, keepdim=True)
            else:
                nn.init.kaiming_normal_(self.res.conv1.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, x, tsne=False,pcl=False):
        x, sfs = self.res(x)
        x = F.relu(x)
        if pcl:
            pcl_feats = x
        ...
        if tsne:
            return seg_output, fea
        elif pcl:
            return seg_output, pcl_feats
