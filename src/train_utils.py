...
def SAM_get_pl_label(images, output_dec1, predictor,
    ...
    for depth in range(output_dec1.shape[0]):
        ...
        for i in range(times):
            for c in range(num_classes):
                result = random_sample(output_dec1[depth, c, :, :].copy())
                if result is not None:
                    points, labels = result
                    masks, scores, logits = predictor.predict(
                        point_coords=points,
                        point_labels=labels,
                        mask_input=mask_input_this_depth[:, c, :, :],
                        multimask_output=True,
                    )
                    this_depth_mask_all[i, c, :, :] = logits.copy()
        ...
        this_depth_argmax_map = (this_depth_probability_map.copy() > 0.5).astype(np.uint8)
