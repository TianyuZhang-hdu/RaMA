...
from dataloaders.normalize import normalize_image, normalize_image_to_0_1_3D
...
class TrainSource:
    def __init__(self, config):
        ...
        self.log_path = os.path.join(config.path_save_log, config.log_name)  # Save Log
        if not os.path.exists(self.model_path):
            os.makedirs(self.model_path)
        if not os.path.exists(self.log_path):
            os.makedirs(self.log_path)
        self.summary_patch = os.path.join(self.log_path, time_now, config.log_label)
        ...
        train_dataset = mms_dataset(config.dataset_root, sr_img_list, sr_label_list,
    ...
    def build_model(self):
        self.model = ResUnet(resnet=self.backbone, num_classes=self.out_ch, pretrained=True, in_ch=self.in_ch).to(self.device)
    ...
    def run(self):
        ...
        losses = [] # loss_meter = meter.AverageValueMeter()
        ...
        for epoch in range(self.num_epochs):
            ...
            losses.clear() # loss_meter.reset()

            for batch, data in enumerate(self.source_train_loader):
                x, y = data['data'], data['mask']
                
                assert np.all(np.mod(y, 1) == 0), \
                    f"ERROR: Mask contains non-integer values! Unique values: {np.unique(y)}"
                
                x = torch.from_numpy(normalize_image_to_0_1_3D(x)).to(dtype=torch.float32).to(self.device)
                ...
                losses.append(loss.item()) # loss_meter.add(loss.item())
            ...
            print(f"Train ———— Total Loss:{epoch_loss:.8f}") # print("Train ———— Total Loss:{:.8f}".format(loss_meter.value()[0]))
            ...
            # Save Model
            if best_loss > epoch_loss: # loss_meter.value()[0]:
                best_loss = epoch_loss # loss_meter.value()[0]
                best_epoch = (epoch + 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Source_Dataset', type=str, default='vendorA',
                        help='vendorA/vendorB/vendorC/vendorD/')
    ...
    # Path
    parser.add_argument('--path_save_log', type=str, default='./repro_training/source_logs/')
    parser.add_argument('--path_save_model', type=str, default='./repro_training/source_models/')
    parser.add_argument('--dataset_root', type=str, default='./data/mnms_png')

    # log
    parser.add_argument('--log_name', type=str, default='train_Source')
    parser.add_argument('--log_label', type=str, default='train_Source')
