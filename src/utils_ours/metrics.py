...
def asd_compute(test, reference):
    batch_size = reference.shape[0]
    lv_asds, myo_asds, rv_asds = [], [], []

    for batch in range(batch_size):
        lv_asd, myo_asd, rv_asd = avg_surface_distance(test=test[batch][0], reference=reference[batch][0]), \
                                  avg_surface_distance(test=test[batch][1], reference=reference[batch][1]), \
                                  avg_surface_distance(test=test[batch][2], reference=reference[batch][2])
            
        lv_asds.append(lv_asd)
        myo_asds.append(myo_asd)
        rv_asds.append(rv_asd)

    return lv_asds, myo_asds, rv_asds
...
def dice_metric(pred, label):
    ...
    lv_dices, myo_dices, rv_dices = [], [], []
    smooth = 1e-6

    for batch in range(batch_size):
        lv_intersection = (pred[batch][0] * label[batch][0]).sum()
        lv_dice = (2 * lv_intersection + smooth) / (pred[batch][0].sum() + label[batch][0].sum() + smooth)

        myo_intersection = (pred[batch][1] * label[batch][1]).sum()
        myo_dice = (2 * myo_intersection + smooth) / (pred[batch][1].sum() + label[batch][1].sum() + smooth)

        rv_intersection = (pred[batch][2] * label[batch][2]).sum()
        rv_dice = (2 * rv_intersection + smooth) / (pred[batch][2].sum() + label[batch][2].sum() + smooth)


        lv_dices.append(lv_dice*100.)
        myo_dices.append(myo_dice*100.)
        rv_dices.append(rv_dice*100.)

    return lv_dices, myo_dices, rv_dices


def calculate_metrics(test, reference):
    test, reference = data_process(pred=test, label=reference, threshold=0.5)
    lv_dice, myo_dice, rv_dice = dice_metric(test, reference)
    lv_asd, myo_asd, rv_asd = asd_compute(test, reference)
    # disc_dis, cup_dis = hd_compute(test, reference)
    return [lv_dice, lv_asd, myo_dice, myo_asd, rv_dice, rv_asd]
